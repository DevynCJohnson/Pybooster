...
override LIST_SCRIPT_PROGRAMS::=alphabetize_lines ampupcpu bin2hex bin2num bin2oct CamelCase char2num cleansystem flipchars genmathart getsysinfo hex2num holograph2str lslibfunc lsmoduleparams man2devhelp minifyxml num2bin num2char num2hex num2oct oct2num PascalCase pipebuf prettifyxml refreshgrub replaceoddchars save_file_times set_access_time setfiletimes str2holograph svgresizer termtest thumbnail-cleaner togglequotes unicalc webget win2unixlines
...
$(BIN)/factorial : $(BIN) $(SRCDIR)/factorial.c $(MACROSPATH)
	$(CC) -DNO_PRINT_ACTION -DNO_PRINT_M -DNO_PRINT_CHARS -DNO_PRINT_FLOATS -DNO_PRINT_WIDTH_PREC -DNO_PRINT_STRINGS -DNO_PRINT_M -DPRINTF_BUF64 -DDDISABLE_ANSI_SGR -DNO_CHAR_TABLES -DNO_SCAN_FLOATS -DDISABLE_MISC_HEADER -DDISABLE_NETWORK_HEADER -D__SINGLE_THREAD__ -DPROGRAM_NAME=\"$(@F)\" $(MINEXE_PARAMS) $(SRCDIR)/$(@F).c -o $@ && $(STRIP) $@
...
$(BIN)/popcount : $(BIN) $(SRCDIR)/popcount.c $(MACROSPATH)
	$(CC) -DNO_PRINT_ACTION -DNO_PRINT_M -DNO_PRINT_CHARS -DNO_PRINT_FLOATS -DNO_PRINT_WIDTH_PREC -DNO_PRINT_STRINGS -DNO_PRINT_M -DPRINTF_BUF32 -DDDISABLE_ANSI_SGR -DNO_CHAR_TABLES -DNO_SCAN_FLOATS -DDISABLE_MISC_HEADER -DDISABLE_NETWORK_HEADER -D__SINGLE_THREAD__ -DPROGRAM_NAME=\"$(@F)\" $(MINEXE_PARAMS) $(SRCDIR)/$(@F).c -o $@ && $(STRIP) $@
...
$(BIN)/number_properties : $(BIN) $(SRCDIR)/number_properties.c $(MACROSPATH)
	$(CC) -DNO_PRINT_ACTION -DNO_PRINT_M -DNO_PRINT_CHARS -DNO_PRINT_WIDTH_PREC -DNO_PRINT_STRINGS -DNO_PRINT_M -DPRINTF_BUF512 -DDDISABLE_ANSI_SGR -DNO_CHAR_TABLES -DNO_SCAN_FLOATS -DDISABLE_MISC_HEADER -DDISABLE_NETWORK_HEADER -D__SINGLE_THREAD__ -DPROGRAM_NAME=\"$(@F)\" $(MINEXE_PARAMS) $(SRCDIR)/$(@F).c -o $@ && $(STRIP) $@
