...
upver :
	...
	# Shell Scripts #
	find . -mount -type f \( -name "*.awk" -o -name "*.sed" -o -name "*.ash" -o -name "*.bash" -o -name "*.ksh" -o -name "*.sh" -o -name "*.zsh" \) -exec sed -i "s/^#' @version 20[0-9][0-9]\.[0-1][0-9]\.[0-3][0-9]/#' @version $(__VERSION__)/; s/^version=20[0-9][0-9]\.[0-1][0-9]\.[0-3][0-9]/version=$(__VERSION__)/; s/^readonly version=20[0-9][0-9]\.[0-1][0-9]\.[0-3][0-9]/readonly version=$(__VERSION__)/" '{}' \;
