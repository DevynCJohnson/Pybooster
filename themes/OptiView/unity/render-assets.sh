...
while read -r filename; do
    if [ -n "${filename}" ] && [ ! -f "${filename}.png" ]; then
        inkscape --export-id="${filename}" --export-id-only --export-png="${filename}.png" assets.svg 2> /dev/null > /dev/null &
    fi
    if [ -n "${filename}" ] && [ ! -f "${filename}@2.png" ]; then
        inkscape --export-id="${filename}" --export-dpi=180 --export-id-only --export-png="${filename}@2.png" assets.svg 2> /dev/null > /dev/null &
    fi
done < assets.txt
