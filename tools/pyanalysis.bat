...
if %FOUND% == 1 (
    pylint --disable="C0301,I0011" --good-names="i,j,k,v,ex,key,Run,val,_" --notes="DEBUG,DEPRECATED,DISABLED,FINISH,FIX,FIXME,REPAIR,TESTING,TODEBUG,TODO,TOMV,TORM,XXX" --msg-template="{path}:{line}: [{msg_id}({symbol}), {obj}] {msg}" --max-module-lines=4096 --max-returns=16 --persistent=n --reports=y %1
) else (
