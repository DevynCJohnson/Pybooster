...
@section SETUP
python3 ./setup.py build && strip --strip-debug --strip-unneeded --remove-section=.comment --remove-section=.note ./build/exe.linux-x86_64-3.6/*.so* && strip --strip-debug --strip-unneeded --remove-section=.comment --remove-section=.note ./build/exe.linux-x86_64-3.6/color-kit && makeself --noprogress --nowait --nox11 ./build/exe.linux-x86_64-3.6/ ./color-kit.run "Loading Color-Kit" ./color-kit
...
if __name__ == '__main__':
    setup(
        ...
        description=r'Convert between color-spaces',
        classifiers=[
            ...
            r'Topic :: Artistic Software',
            r'Topic :: Multimedia :: Graphics'
        ],
        executables=[
            Executable(
                script=r'../scripts/color-kit',
                initScript=None,
                base=PLAT,
                targetName=None,
                icon=joinpath(dirname(__file__), r'../themes/Opticons/apps/color-kit.svg'),
                shortcutName=None,
                shortcutDir=None,
                copyright=r'LGPLv3',
                trademarks=None
