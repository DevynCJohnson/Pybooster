...
install_deb_pkgs() {
    if [ -x "$(command -v apt-get)" ]; then
        apt-get --quiet --yes install astyle bcpp binwalk bsdiff cccc cgdb clang cloc colormake complexity coreutils cppcheck cppreference-doc-en-html csstidy debmake devhelp doschk doxygen doxygen-gui flawfinder gcc gdb git-cola git-doc git-extras git-flow git-gui gitk gitlint gitstats glade indent jsonlint kwstyle libxml2-utils licensecheck llvm ltrace make moreutils optipng patchutils perltidy pmccabe pngcrush pscan python3-chardet python3-demjson python3-gi python3-logilab-common python3-pip python3-pytest python3-pytest-pep8 qgit shc shellcheck sloccount splint strace tidy uchardet uncrustify undertaker universalindentgui vbindiff wamerican-insane xdg-utils xmlindent xxdiff xxdiff-scripts xxgdb yajl-tools
